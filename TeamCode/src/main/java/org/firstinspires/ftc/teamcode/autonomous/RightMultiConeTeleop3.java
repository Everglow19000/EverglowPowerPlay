...
@Autonomous(name = "RightMultiConeTeleop3", group = "Template")
public class RightMultiConeTeleop3 extends LinearOpMode {
    @Override
    public void runOpMode() {
        ...
        Pose dropOffLocation = new Pose(left * (1.5 * TILE_SIZE - 22) + 8, -0.5 * TILE_SIZE + 12 + 10, toRadians(-45));
        ...
        Trajectory startTrajectoryNew = new Trajectory(
                new SplinePath(new PointD[]{
                        new PointD(startPosition.x + 20, startPosition.y),
                        new PointD(left * 1.5 * TILE_SIZE, -2 * TILE_SIZE),
                        new PointD(left * 1.5 * TILE_SIZE, - 0.8 * TILE_SIZE),
                        new PointD(left * (1 * TILE_SIZE + 5), - 0.4 * TILE_SIZE - 5 + 2)
        ...
        waitForStart();
//        systems.verifyStopSequence.enable();
